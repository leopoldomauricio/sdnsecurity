...
cliente05='10.224.70.50'
servidor05='10.224.76.198'
test_name = 
...
for ((i=0; i<=END2; i++)); do
   ...
   sshpass -p "ChangeMe" ssh -o StrictHostKeyChecking=no root@${cliente05} export servidor05='10.224.76.198'
...
# iperf
   ...
   for j in {1..10}; do
      if [ $j -lt 10 ]; then
         sshpass -p "ChangeMe" ssh root@${cliente05} "iperf -c $servidor05 -p $(($i+20000)) -b ${bw} -u -l 1334 | grep '%)'" | awk '{print $7;}' | tr '\n' ',' >> $3
         #sshpass -p "ChangeMe" ssh root@${cliente05} "iperf -c $servidor05 -p $(($i+20000)) -b ${bw} -u -l 1334 | grep '%)'" | awk '{print $7;}' | tr '\n' ',' >> leo-10a20000regras-iptables-onenode-THROUGHPUT.csv
         #sshpass -p "ChangeMe" ssh root@${cliente05} "iperf -c $servidor06 -p $(($i+20000)) -b ${bw} -u -l 1334 | grep '%)'" | awk '{print $7;}' | tr '\n' ',' >> leo-10a20000regras-iptables-twonodes-THROUGHPUT.csv
      else
         sshpass -p "ChangeMe" ssh root@${cliente05} "iperf -c $servidor05 -p $(($i+20000)) -b ${bw} -u -l 1334 | grep '%)'" | awk '{print $7;}' >> $3
         #sshpass -p "ChangeMe" ssh root@${cliente05} "iperf -c $servidor05 -p $(($i+20000)) -b ${bw} -u -l 1334 | grep '%)'" | awk '{print $7;}' >> leo-10a20000regras-iptables-onenode-THROUGHPUT.csv
         #sshpass -p "ChangeMe" ssh root@${cliente05} "iperf -c $servidor06 -p $(($i+20000)) -b ${bw} -u -l 1334 | grep '%)'" | awk '{print $7;}' >> leo-10a20000regras-iptables-twonodes-THROUGHPUT.csv
      fi
